...
export class AccountRepository {
  ...
  async create(input: CreateAccountInput): Promise<SerializedAccount> {
    if ((input.balance ?? 0) > 0) {
      throw new Error('Initial account balance must be created through ledger entries');
    }
  ...
  async transferBalance(fromAccountId: string, toAccountId: string, amount: number): Promise<void> {
